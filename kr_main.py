...
#
# ★ 연기금이 없는 이유: 연기금은 기관의 하위 분류라 네이버가 공개하지 않는다.
#   data.krx.co.kr 은 2026 년에 로그인·OpenAPI 의무화로 막혔다 - 세션 쿠키를 붙여도
#   모든 getJsonData 호출이 400 LOGOUT 이고 기본 시세 조회까지 같다. 데이터에 미제공
#   사실을 실어 브리핑이 연기금을 지어내지 못하게 한다.
#
#   ★★ 나중에 붙일 때 쓸 것 (2026-09-20 조사 완료, 다시 찾지 말 것):
#   - KRX OpenAPI(openapi.krx.co.kr)는 답이 아니다. 서비스 목록에 투자자별 항목이
#     아예 없다 - 주식 분류는 일별매매정보와 종목기본정보뿐이다. 가입해도 안 나온다.
#   - 답은 한국투자증권 KIS Developers 의 '국내기관_외국인 매매종목가집계'
#     (국내주식-037, /uapi/domestic-stock/v1/quotations/foreign-institution-total).
#     종목별로 아래를 '거래대금' 실값으로 준다 - 지금처럼 수량×종가로 추정하지 않아도
#     되고, 시총 상위로 모집단을 한정하지 않아도 된다:
#       frgn_ntby_tr_pbmn 외국인 / orgn_ntby_tr_pbmn 기관계 / ivtr_ 투자신탁 /
#       bank_ 은행 / insu_ 보험 / mrbn_ 종금 / etc_orgt_ 기타단체 / etc_corp_ 기타법인 /
#       ★ fund_ntby_tr_pbmn = 기금 → KRX 분류의 '연기금등'
#     시장 전체는 '시장별 투자자매매동향(일별)'(inquire_investor_daily_by_market)이
#     사모·기금까지 분리해 준다.
#   - 전제: 한투 계좌 + 앱키(APP_KEY/APP_SECRET)로 OAuth 토큰을 받아 쓴다. 사용자는
#     계좌와 키를 갖고 있으나 2026-09-20 기준 접속 불가 상태라 보류했다.
#   - 가집계(추정)라는 점은 그대로 표기해야 한다.
#
